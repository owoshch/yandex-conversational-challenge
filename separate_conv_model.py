...
class NERModel(BaseModel):
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("proj"):
            ...
            logits_expanded = tf.expand_dims(logits, 2)
            
            
            
            with tf.variable_scope('conv1') as scope:
                w = tf.get_variable('weights', [5, 5, self.config.hidden_dense_dim, self.config.hidden_dense_dim])
                b = tf.get_variable('biases', [self.config.hidden_dense_dim], 
                                    initializer=tf.random_normal_initializer())
                conv = tf.nn.conv2d(logits_expanded, w, strides=[1, 1, 1, 1], padding='SAME')
                conv1 = tf.nn.relu(conv + b, name=scope.name)
                
            
            conv1_squeezed = tf.squeeze(conv1, 2)
            
            
            logits = tf.reduce_mean(conv1_squeezed, axis=1)
        ...
        with tf.variable_scope("proj-replies"):
            ...
            logits_reply_expanded = tf.expand_dims(logits_reply, 2)
            
            
            
            with tf.variable_scope('conv_reply') as scope_reply:
                w_reply = tf.get_variable('weights', [5, 5, self.config.hidden_dense_dim, self.config.hidden_dense_dim])
                b_reply = tf.get_variable('biases', [self.config.hidden_dense_dim], 
                                    initializer=tf.random_normal_initializer())
                conv_reply = tf.nn.conv2d(logits_reply_expanded, w_reply, strides=[1, 1, 1, 1], padding='SAME')
                conv1_reply = tf.nn.relu(conv_reply + b_reply, name=scope_reply.name)
                
            conv1_reply_squeezed = tf.squeeze(conv1_reply, 2)
            
            
            #logits_reply = tf.reduce_mean(logits_reply, axis=1)
            
            logits_reply = tf.reduce_mean(conv1_reply_squeezed, axis=1)
