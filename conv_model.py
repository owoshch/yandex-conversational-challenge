...
class NERModel(BaseModel):
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("proj"):
            ...
            #print ('expanded logits shape', logits_expanded.shape)
            
            with tf.variable_scope('conv1') as scope:
                w = tf.get_variable('weights', [25, 25, self.config.hidden_dense_dim, self.config.hidden_dense_dim])
            ...
            with tf.variable_scope('conv2') as scope2:
                w2 = tf.get_variable('weights_2', [10, 10, self.config.hidden_dense_dim, self.config.hidden_dense_dim])
                b2 = tf.get_variable('biases_2', [self.config.hidden_dense_dim], 
                                    initializer=tf.random_normal_initializer())
                conv2 = tf.nn.conv2d(conv1, w2, strides=[1, 1, 1, 1], padding='SAME')
                conv2 = tf.nn.relu(conv2 + b2, name=scope2.name)
            ...
            conv2_squeezed = tf.squeeze(conv2, 2)
            
            #print ('conv1 squeezed', conv1_squeezed.shape)
            
            #logits = tf.reduce_mean(logits, axis=1)
            
            logits = tf.reduce_mean(conv2_squeezed, axis=1)
            
            #print ('final logits shape', logits.shape)
            
            
            W1 = tf.get_variable("W1", dtype=tf.float32,
                    shape=[self.config.hidden_dense_dim, 1])
            ...
            print ('squeezed sc shape', sc.shape)
