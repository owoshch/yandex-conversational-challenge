...
from model.data_utils import UNK, NUM, BEGIN, END, \
    ...
    merge_context_and_reply, export_trimmed_fasttext_vectors

config = Config(load=False)

print ('Loading train from', config.train_indices)
train_ids = np.load(config.train_indices)
print ('Loading test from', config.test_indices)
...
public = pd.read_csv(config.path_to_private_dataframe, error_bad_lines=False, sep = '[  . ? , !]?\t', 
                   header=None)
public.columns = config.test_column_names



print ('Loading vocab from', config.filename_words_final)
vocab = load_vocab(config.filename_words_final)


print ('Loading unk dictionary from', config.final_unk_dict)
unk_dict = np.load(config.final_unk_dict).item()
...
data.to_csv(config.path_to_final_preprocessed_train, index=False)
public.to_csv(config.path_to_final_preprocessed_test, index=False)
